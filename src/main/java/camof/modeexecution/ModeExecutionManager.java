...
import camof.modeexecution.mobilitymodels.modehelpers.StartHelper;
...
public class ModeExecutionManager {
    ...
    public static void startModes(String configPath) throws Exception {
        ...
        if(config.keySet().contains("modes") && config.keySet().contains("modejson") && new File(config.getString("modejson")).exists()){
            if(config.get("modes") instanceof JSONArray modesArray){
                ...
                for(String mode : modes){
                    executeMode(mode,agents,modejson.getJSONObject(mode).getString("resultsFolder"));
                }
    ...
    public static void executeMode(String modeName, List<Agent> agents, String resultPath){
    ...
    public static MobilityMode findMode(String modeName){
        try {
            Class<?> modeClass = Class.forName("camof.modeexecution.mobilitymodels."+ ModeExecutionManager.modeValues.get(modeName).getString("class name"));
